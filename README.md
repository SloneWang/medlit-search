...
> **推荐从 `v1.6` 稳定分支安装**（见下方"稳定版本"一节）。

## 稳定版本

**请从 `v1.6` 稳定分支安装**，`main` 分支为开发分支，可能包含未充分验证的改动；`v1.6` 与更早的稳定分支（`v1.5` / `v1.4`）已锁定，禁止直接修改。
...
| `v1.6` | **当前稳定分支（推荐安装）**：强制参数确认、中文分步文件夹、默认日期降序、纳排标准、`_revN` 修订机制、固定页大小 20 + 超长检索式自动 POST、IO 协程化；**已锁定** |
| `v1.5` | 早期稳定分支（逐步中断六步工作流 + 多源合并去重/全文准备/审查表）；**已锁定** |
| `v1.4` | 更早稳定版（全量类型注解，无逐步中断流程）；**已锁定** |

## 更新日志

### v1.6（2026-09-17，已签出 v1.6 稳定分支）
